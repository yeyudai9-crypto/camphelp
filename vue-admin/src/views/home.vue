...
<div class="content" :style='{"minHeight":"100vh","padding":"0px 20px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230830/d58ecc944fb54dbfa18ecd2b64e92170.png) no-repeat center top / cover","display":"flex","fontSize":"16px"}'>
	...
	<div :style='{"margin":"0 20px 30px 0","alignItems":"center","color":"#fff","flexWrap":"wrap","display":"flex","width":"240px","fontSize":"inherit","justifyContent":"center"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 10px 16px 0","borderRadius":"4px","background":"rgba(86,178,198,.8)","display":"flex","clipPath":"polygon(95% 0, 100% 50%, 95% 100%, 0% 100%, 0 50%, 0% 0)","width":"100%"}' v-if="isAuth('kuaididingdan','首页总数')">
		...
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 10px 16px 0","borderRadius":"4px","background":"rgba(86,178,198,.6)","display":"flex","clipPath":"polygon(95% 0, 100% 50%, 95% 100%, 0% 100%, 0 50%, 0% 0)","width":"100%"}' v-if="isAuth('daiqupingjia','首页总数')">
			...
			<div :style='{"width":"calc(100% - 60px)","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{daiqupingjiaCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>代取评价总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 2 -->
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","margin":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"460px"}'>
		<div id="kuaididingdanChart1" class="echarts1" v-if="isAuth('kuaididingdan','首页统计')"></div>
		<div id="daiqupingjiaChart1" class="echarts2" v-if="isAuth('daiqupingjia','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getkuaididingdanCount();
		this.kuaididingdanChat1();
		this.getdaiqupingjiaCount();
		this.daiqupingjiaChat1();
	},
	methods:{
		// 词云
		...
		getkuaididingdanCount() {
			this.$http({
				url: `kuaididingdan/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.kuaididingdanCount = data.data
				}
			})
		},
		kuaididingdanChat1() {
        ...
        var kuaididingdanChart1 = echarts.init(document.getElementById("kuaididingdanChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                kuaididingdanChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    kuaididingdanChart1.resize();
                };
            }
        });
      })
		...
		getdaiqupingjiaCount() {
			this.$http({
				url: `daiqupingjia/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.daiqupingjiaCount = data.data
				}
			})
		},
		daiqupingjiaChat1() {
        ...
        var daiqupingjiaChart1 = echarts.init(document.getElementById("daiqupingjiaChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                daiqupingjiaChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    daiqupingjiaChart1.resize();
                };
            }
        });
      })
