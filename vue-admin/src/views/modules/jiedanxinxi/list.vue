<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"fontSize":"inherit","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.kuaidileixing" placeholder="快递类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiedanxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-kaitongfuwu" :style='{"margin":"0 2px","fontSize":"inherit","color":"#fff","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('jiedanxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"0","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="peisongzhanghao"
						label="配送账号">
						<template slot-scope="scope">
							{{scope.row.peisongzhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lianxidianhua"
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="kuaidizhuangtai"
						label="快递状态">
						<template slot-scope="scope">
							{{scope.row.kuaidizhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiedanshijian"
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="配送人">
						<template slot-scope="scope">
							{{scope.row.peisongren}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							<span style="margin-right:10px">{{scope.row.ispay=='已支付'?'已支付':'未支付'}}</span>
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('jiedanxinxi','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiedanxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-dagou" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiedanxinxi','送达')" @click="songdadingdanCrossAddOrUpdateHandler(scope.row,'cross','','kuaidizhuangtai','已送达','已送达,送货中'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								送达
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiedanxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jiedanxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<songdadingdan-cross-add-or-update v-if="songdadingdanCrossAddOrUpdateFlag" :parent="this" ref="songdadingdanCrossaddOrUpdate"></songdadingdan-cross-add-or-update>
...
import songdadingdanCrossAddOrUpdate from "../songdadingdan/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				songdadingdanCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			songdadingdanCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				if(row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.songdadingdanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiedanxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.songdadingdanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.songdadingdanCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				let params = {
					tradeno: row.dingdanhao,
					subject: row.kuaidimingcheng,
					totalamount: row.daiqufeiyong,
				}
				this.$http({
					url: "jiedanxinxi/alipay",
					method: "get",
					params: params
				}).then(({ data }) => {
						if (data && data.code === 0) {
							var newWin = window.open('_blank');
							newWin.document.querySelector('body').innerHTML = data.data;
	...
	jiedanxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jiedanxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
