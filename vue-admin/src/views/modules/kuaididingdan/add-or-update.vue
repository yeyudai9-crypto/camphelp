	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' class="select" v-if="type!='info'"  label="快递类型" prop="kuaidileixing">
					<el-select :disabled="ro.kuaidileixing" v-model="ruleForm.kuaidileixing" placeholder="请选择快递类型" >
						<el-option
				...
				<el-form-item :style='{"color":"inherit","margin":"0 0 20px 0","fontSize":"inherit"}' v-else class="input" label="快递类型" prop="kuaidileixing">
					<el-input v-model="ruleForm.kuaidileixing"
						placeholder="快递类型" readonly></el-input>
				</el-form-item>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kuaididingdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kuaididingdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
