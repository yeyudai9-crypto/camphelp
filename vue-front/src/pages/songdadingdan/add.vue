...
<div :style='{"width":"60%","padding":"20px","margin":"0 auto","overflow":"hidden","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}' label="送达时间" prop="songdashijian">
              <el-date-picker
				  :disabled=" false  ||ro.songdashijian"
                  value-format="yyyy-MM-dd HH:mm:ss"
                  v-model="ruleForm.songdashijian" 
                  type="datetime"
                  placeholder="送达时间">
              </el-date-picker>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='songdashijian'){
              this.ruleForm.songdashijian = obj[o];
              this.ro.songdashijian = true;
              continue;
            }
            if(o=='peisongren'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`songdadingdan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('songdadingdan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`songdadingdan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
