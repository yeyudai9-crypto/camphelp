...
<div :style='{"width":"60%","padding":"20px","margin":"0 auto","overflow":"hidden","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}'  label="快递类型" prop="kuaidileixing">
            <el-select v-model="ruleForm.kuaidileixing" placeholder="请选择快递类型"  >
              <el-option
                  v-for="(item,index) in kuaidileixingOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}'  label="状态" prop="zhuangtai">
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.zhuangtaiOptions = "已被接单,待接单".split(',')
      },

    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`kuaididingdan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('kuaididingdan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`kuaididingdan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
