...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"120px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 220px)","float":"left","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>快递类型：</div>
        <el-input v-model="formSearch.kuaidileixing" placeholder="快递类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	  ...
	  <el-button v-if="btnAuth('daiqupingjia','新增')" :style='{"cursor":"pointer","border":"1px solid #170000","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/daiqupingjiaAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","alignItems":"center","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"auto","padding":"0 10px 0 0","flexDirection":"column","flex":"1","display":"flex"}'>
				<div class="title">{{item.peisongzhanghao}}</div>
				<div class="title">评分:{{item.pingfen}}</div>
		      ...
		      <div class="desc">{{item.pingjia}}</div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`daiqupingjia/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/daiqupingjiaDetail', query: params});
      },
